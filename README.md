...
| [WAF Offenders](./waf/) | Source data | Source IPs blocked at an edge reverse proxy. Pushed directly by the proxy, not generated by a workflow. Folded into the combined threat feed, so it does not need its own EDL object. | `waf/waf-offenders.txt` |
...
The WAF offender list is pushed hourly by the edge proxy. Each push triggers `update-waf-readme.yml` to regenerate `waf/README.md`'s stats, and the list itself is picked up by the threat feed workflow on its next 04:30 UTC run.
...
gcp/
  gcp-ranges.txt            # All GCP IPv4/IPv6 prefixes, collapsed
  README.md                 # Raw/aggregated counts, Google syncToken, last updated
lolrmm/
...
waf/
  ...
  README.md                # Auto-regenerated on push to waf-offenders.txt
.github/workflows/
  update-threat-edl.yml
  update-asn-edl.yml
  update-gcp-edl.yml
  update-lolrmm-edl.yml
  update-waf-readme.yml
```
